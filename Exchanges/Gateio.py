...
class Gateio(Exchange):
    ...
    def set_future_batch_orders(self, settle, orders):
        ...
        for order in orders:
            data = {}
            data['contract'] = str(order['contract'])
            data['size'] = str(order['size'])
            if order.has_key('iceberg'):
                data['iceberg'] = str(order['iceberg'])
            if order.has_key('price'):
                data['price'] = str(order['price'])
            if order.has_key('close'):
                data['close'] = str(order['close'])
            if order.has_key('reduce_only'):
                data['reduce_only'] = str(order['reduce_only'])
            if order.has_key('tif'):
                data['tif'] = str(order['tif'])
            if order.has_key('text'):
                data['text'] = str(order['text'])
            if order.has_key('auto_size'):
    ...
    def update_future_ofders_with_id(self, settle, order_id, size, price):
        self.url = '/futures/' + str(settle) +'/orders/' + str(order_id)
        self.query_param = '' 
        data = {}
        data['size'] = size
